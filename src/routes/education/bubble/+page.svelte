<script lang="ts">
	...
	const codeLines: string[] = ['function bubbleSort(arr) {', '    let n = arr.length;', '    for (let i = 0; i < n - 1; i++) {', '        for (let j = 0; j < n - i - 1; j++) {', '            if (arr[j] > arr[j+1]) {', '                let temp = arr[j];', '                arr[j] = arr[j+1];', '                arr[j+1] = temp;', '            }', '        }', '    }', '}'];
	...
	function getSortedIndices(i: number, n: number): number[] {
		let sorted: number[] = [];
		for (let k = n - i; k < n; k++) {
			sorted.push(k);
		}
	...
	function generateStates(): State[] {
		...
		let allSorted: number[] = Array.from({ length: n }, (_, i) => i);
	...
	<div class="visualizer-container">
				...
				<div class="panel-content">
					<div class="bars-container">
						{#each currentState.arr as val, index}
							<div class="bar-wrapper">
								<div
									class="bar
                                    {currentState.swapping.includes(index) ? 'bg-swapping' : currentState.comparing.includes(index) ? 'bg-comparing' : currentState.sorted.includes(index) ? 'bg-sorted' : 'bg-unsorted'}"
					...
					<div class="legend">
						<div class="legend-item">
							<div class="color-box bg-unsorted"></div>
							unsorted
						</div>
						<div class="legend-item">
							<div class="color-box bg-comparing"></div>
							comparing
						</div>
						<div class="legend-item">
							<div class="color-box bg-swapping"></div>
							swapping
						</div>
						<div class="legend-item">
							<div class="color-box bg-sorted"></div>
							sorted
						</div>
					</div>
				</div>
		...
		<div class="controls-row">
			<div class="playback-controls">
				...
				<button class="btn" on:click={step} disabled={isPlaying}> Step &rarr; </button>
			</div>
