<script lang="ts">
    ...
    let states = $state<{ arr: number[]; line: number; comparing: number[]; swapping: number[]; sorted: number[] }[]>([]);
    let currentStep = $state(0);
    let isPlaying = $state(false);
    let speed = $state(100);
    let timer: any;
    let autoLoopTimer: any;
    ...
    function highlightCode(line: string) {
        ...
        hl = hl.replace(/\b(selectionSort|length)\b/g, '<span class="syntax-function">$1</span>');
        hl = hl.replace(/\b(arr|n|i|j|min_idx|temp)\b/g, '<span class="syntax-variable">$1</span>');
    ...
    function generateStates() {
        ...
        for (let i = 0; i < n - 1; i++) {
            ...
            let min_idx = i;
            newStates.push({ arr: [...tempArr], line: 4, comparing: [min_idx], swapping: [], sorted: sortedArr });
            
            for (let j = i + 1; j < n; j++) {
                newStates.push({ arr: [...tempArr], line: 5, comparing: [min_idx, j], swapping: [], sorted: sortedArr });
                newStates.push({ arr: [...tempArr], line: 6, comparing: [min_idx, j], swapping: [], sorted: sortedArr });
                
                if (tempArr[j] < tempArr[min_idx]) {
                    min_idx = j;
                    newStates.push({ arr: [...tempArr], line: 7, comparing: [min_idx], swapping: [], sorted: sortedArr });
                }
                newStates.push({ arr: [...tempArr], line: 8, comparing: [min_idx], swapping: [], sorted: sortedArr });
            }
            newStates.push({ arr: [...tempArr], line: 9, comparing: [min_idx], swapping: [], sorted: sortedArr });
            
            newStates.push({ arr: [...tempArr], line: 10, comparing: [i, min_idx], swapping: [], sorted: sortedArr });
            if (min_idx !== i) {
                newStates.push({ arr: [...tempArr], line: 11, comparing: [], swapping: [i, min_idx], sorted: sortedArr });
                
                let temp = tempArr[i];
                tempArr[i] = tempArr[min_idx];
                newStates.push({ arr: [...tempArr], line: 12, comparing: [], swapping: [i, min_idx], sorted: sortedArr });
                
                tempArr[min_idx] = temp;
                newStates.push({ arr: [...tempArr], line: 13, comparing: [], swapping: [i, min_idx], sorted: sortedArr });
            }
            newStates.push({ arr: [...tempArr], line: 14, comparing: [], swapping: [], sorted: sortedArr });
        }
        
        let allSorted = Array.from({length: n}, (_, k) => k);
        newStates.push({ arr: [...tempArr], line: 15, comparing: [], swapping: [], sorted: allSorted });
        newStates.push({ arr: [...tempArr], line: 16, comparing: [], swapping: [], sorted: allSorted });
...
<main class="main">
    ...
    <header class="header">
        <div class="line"></div>
        <h1 class="text-4xl font-black text-white uppercase">Selection Sort</h1>
        <p>Selection Sort คืออัลกอริทึมการเรียงลำดับที่ทำงานโดยการค้นหาค่าที่น้อยที่สุด (หรือมากที่สุด) จากส่วนที่ยังไม่ได้เรียงลำดับในอาเรย์ แล้วนำมาสลับกับสมาชิกตัวแรกสุดของส่วนนั้น กระบวนการนี้จะทำซ้ำไปเรื่อยๆ จนกว่าทุกองค์ประกอบจะถูกจัดเรียงอย่างถูกต้อง แม้จะเข้าใจง่ายและใช้หน่วยความจำเพิ่มเพียงเล็กน้อย แต่ก็ไม่มีประสิทธิภาพสำหรับข้อมูลขนาดใหญ่เนื่องจากต้องสแกนข้อมูลที่เหลือตลอดเวลา</p>
    </header>
    <!-- Grid -->
    <div class="layout">
        <!-- Left -->
        <section class="left">
            <div class="card">
                ...
                <div class="bar" style="width: 100%; height: 6px; background: rgba(255, 255, 255, 0.1); border-radius: 3px; margin-top: 5px; overflow: hidden;"><div class="fill green small" style="width: 100%; height: 100%; background: #6bfad8;"></div></div>
            ...
            <div class="glass">
                <p>Selection Sort มีความซับซ้อนเวลาเท่ากันเสมอ ไม่ว่าข้อมูลจะสุ่มมาอย่างไร เหมาะกับระบบที่มีหน่วยความจำจำกัด</p>
            </div>
        </section>
                    ...
                    <div class="protocol-grid">
                        <div class="protocol-item">
                            ...
                            <div class="protocol-content">
                                <h4>Find Minimum</h4>
                                <p>เริ่มต้นโดยกำหนดตำแหน่งแรกสุดว่ายังไม่เรียงลำดับ แล้วสแกนหาค่าที่น้อยที่สุดในข้อมูลที่เหลือ</p>
                            </div>
                        </div>
                        <div class="protocol-item">
                            ...
                            <div class="protocol-content">
                                <h4>Data Swapping</h4>
                                <p>เมื่อพบค่าตัวที่น้อยที่สุด จะนำสลับที่กับข้อมูลตัวแรกสุดในเขตแดนของส่วนที่ยังไม่ได้เรียงลำดับ</p>
                            </div>
                        </div>
                        <div class="protocol-item">
                            ...
                            <div class="protocol-content">
                                ...
                                <p>ขยับเส้นแบ่งส่วนเรียงลำดับไป 1 ตำแหน่ง แล้วทำซ้ำขั้นตอนย่อยนี้ไปเรื่อยๆ จนกว่าข้อมูลทุกตัวจะถูกเรียง</p>
                            </div>
                        </div>
                        <div class="protocol-item">
                            ...
                            <div class="protocol-content">
                                <h4>Instability</h4>
                                <p>Selection Sort ไม่ใช่ Stable Sort เพราะการสลับข้ามตำแหน่งไกลๆ อาจทำลายลำดับสัมพัทธ์เดิมของค่าที่ซ้ำกันได้</p>
                            </div>
                        </div>
                    </div>
            </div>
